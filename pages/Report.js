...
const Report = ({ route }) => {
  ...
  useEffect(() => {
    const groupedByDateThenCategory = transactions.reduce((acc, curr) => {
        ...
        const transformedData = Object.keys(groupedByDateThenCategory).map(date => {
            const categoriesForDate = groupedByDateThenCategory[date];
            return {
              title: date,  // Date is now the section title
              data: Object.keys(categoriesForDate).map(category => ({
                category: category,
                transactions: categoriesForDate[category], // Array of transactions for this category
              }))
            };
          });
  ...
  const renderItem = ({ item }) => (
    <View>
      ...
      {item.transactions.map((transaction, index) => (
        <View style={styles.reportItem} key={transaction.date + transaction.description + index}>
